...
class UploadHandler {
        ...
        # non-chunked upload
            ...
            if ($target){
                ...
                if (move_uploaded_file($file['tmp_name'], $target)){
                    //error_log("uploaded [$target] ext[$ext]");
                    if ($ext == 'wav') {
                        $newtarget = str_replace('.wav','.mp3',$target);
                        //error_log("new[$newtarget] old[$target]");
                        exec("ffmpeg -y -i $target $newtarget",$output,$result);
                        //error_log("output[".print_r($output,true)."] result[$result]");
                        if (!$result && file_exists($newtarget)) {
                            $this->uploadName = basename($newtarget);
                            unlink($target);
                        }
                    }
                    $_SESSION['u_files'][$uuid] = str_replace($uploadDirectory,'',$target);
                    return array('success'=> true, "uuid" => $uuid);
                }
            }
     ...
     * @params string $type what we delete - image or audio
     *
     */
    public function handleDelete($uploadDirectory, $name=null,$type="image")
    {
        if ($this->isInaccessible($uploadDirectory)) {
	    ...
	    if ($method == "DELETE") {
            ...
            $userFolder = in_array('uf',$tokens) ? true : false;
        } else if ($method == "POST") {
            $uuid = $_REQUEST['qquuid'];
            $userFolder = empty($_REQUEST['uf']) ? false : true;
        } else {
        ...
        if(!$userFolder)
        {
            if(empty($_SESSION['u_files'][$uuid]))
            {
                return array("success" => false,
                    "error" => "File not found in session! Unable to delete.".$url,
                    "path" => $uuid
                );
        
            }
            $file =  $_SESSION['u_files'][$uuid];
        }else{
            $file = base64_decode($uuid) ;
            
           
            //@: not good! Need refactoring
            
            $targetFolder = str_replace('/images/user_images','',$targetFolder);
        }
        
        //@todo: need to add restiction check
        $target = join(DIRECTORY_SEPARATOR, array($targetFolder, $file));
