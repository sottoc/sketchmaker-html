    ...
    switch(element.className)
    {
        ...
        case 'Text':
            if(this.stage.attrs.hand_write)
            {
                ...
                if(context._hand)
                {
                    var parent = context._hand.parent;
                    context._hand.destroy();
                    context._hand = null;
                    if(parent && parent.getName =='transition_group')
                    {
                        parent.destroy();
                    }
                }
...
CanvasRecorder.prototype.writeElementWithHand = function(element)
{
    ...
    imageObj.onload = function() {
        ...
        var fullDuration = (parseFloat(element.parent.attrs.beforeNext)  * 1000 + parseFloat(element.parent.attrs.delay)  * 1000 + parseFloat(context.animationDuration) * 1000)- context.animation.frame.time;
        
        var animationStartTime = Date.now();
        
        
        //console.log('durationFull',fullDuration);
        //console.log('text.length',text.length);
        var duration =  ~~(~~(fullDuration)  / text.length ) ;
        ...
        if(context.capturer)
        {
            ...
            duration = (~~(((fullDuration - text.length *frameRate) / text.length)  / frameRate)) * frameRate;
            tweenDuration = duration / 1000;
            //console.log('durationRecorder', duration);
            //console.log('tweenDurationRecorder',tweenDuration);
            //console.log('startTime:',Date.now()-animationStartTime);
        }
        ...
        function type_text() {
            ...
            if (textLength < text.length + 1) {
                ...
                pos = element.parent.position();
                var tween = new context.Konva.Tween({
                    node: hand,
                    x: element.textArr[textRow].width,
                    y: newRowOffsetY+fontHeight/2 + ((fontHeight/2)*direction*Math.random()),
                    ...
                    onFinish: function(){
                        //handX = hand.getX();
                        //handY = hand.getY();                        
                        //console.log('LetterAnimationFinishTime:',Date.now() - animationStartTime);
                        if(textLength >= text.length + 1)
                        {
                ...
                //console.log('LetterAnimationStartTime:',Date.now() - animationStartTime);
                tween.play();
        ...
        //Apply shake effect to hand
        var fullShakeCycleDuration = 1;
        var transformArray = [
            {x:1,y:1,rotate:0},
            {x:-1,y:-2,rotate:-1},
            {x:-3,y:0,rotate:1},
        ...
        var currentShakeStep = 0;
        var shakeStepDuration = fullShakeCycleDuration/ transformArray.length;
        
        function shakeHand(animationGroup){
            if(animationGroup.parent == undefined){
                return false;
            }
            if(currentShakeStep >= transformArray.length)
            {
                currentShakeStep = 0;
            }
            
            animationGroup.to({
                x:  transformArray[currentShakeStep].x*2,
                y:  transformArray[currentShakeStep].y*2,
                rotation: transformArray[currentShakeStep].rotate*2,
                duration:shakeStepDuration,
                onFinish: function(){
                    currentShakeStep++;
                    shakeHand(animationGroup);
                }
            });
        ...
        var handX = animationGroup.x();
        var handY = animationGroup.y();        
...
CanvasRecorder.prototype.drawSvgElementWithHand = function(element){
    ...
    console.log('strokeThikness',strokeThikness);
...
CanvasRecorder.prototype.preloadSvg = function(url,konvaObj)
{
    ...
    return context.$.get(url,function(data) {
        ...
        if(konvaObj && paths.length <=3)
        {
            ...
            if(konvaObj.parent!==undefined && (konvaObj.parent.attrs.boldness !== undefined  || context.capturer!==undefined   )   )
            {
                konvaObj.parent.attrs.boldness = konvaObj.parent.attrs.boldness || 1; 
